...
<script>
    $(document).ready(function() {
        ...
        function tick() {
            if (video.readyState === video.HAVE_ENOUGH_DATA) {
                ...
                if (code) {
                    ...
                    //스트링값 암호화되어서 적어도 15이상일테니
                    //아무 제한 조건이 없으면 그냥 허공에서도 감지해서 자동으로 ajax통신하고 멈춰버림
                    if(code.data.length>=15) {
                        $('#outputData').val(code.data);
                        $('#outputData').trigger("change");
                        //hidden value change에 trigger
                        return;
                    }
                }
                // QR코드 인식에 실패한 경우
        ...
        //outputData가 hidden이라 value Change에 Event 걸어도 작동X
        //change와 펑션 bind
