...
<script>
    let onechat = {
        ...
        loadMessagesFromStorage: async function () {
            const messages = JSON.parse(localStorage.getItem("messages")) || [];
                ...
                if (msg.body) {
                    ...
                    await new Promise((resolve, reject) => {
                        $.ajax({
                            url    : '/getstdnimg',
                            method : 'GET',
                            data   : {stdnId: parseMsg.sendid}, // 이미지를 가져올 학생의 ID를 전달합니다.
                    ...
                    // console.log(parseMsg.receiveid + ": receiveid");
                    // console.log(parseMsg.content + ": content")
                    // // $("#message").append("내가 받은 메세지"+parseMsg.sendid+parseMsg.receiveid+parseMsg.content+"</br>")
                    // $("#message").append(
                    //     '<div class="d-flex justify-content-start mb-10"><div class="d-flex flex-column align-items-start"> <div class="d-flex align-items-center mb-2"> <div class="symbol symbol-35px symbol-circle"> <img alt="Pic" src="/uimg/logo.png"/> </div> <div class="ms-3"> <a href="#" class="fs-5 fw-bold text-gray-900 text-hover-primary me-1">학생이름<span style="font-weight: 200; font-size: small; color:gray"> @' + parseMsg.sendid + '</span></a></div> </div> <div class="p-5 rounded bg-light-info text-dark fw-semibold mw-lg-400px text-start">' + parseMsg.content+ '</div></div></div>'
                    // );
                } else {
                    // 내가 보낸 메시지 파싱
                    ...
                    // $("#message").append("내가 보낸 메시지"+parseMsg2.sendid+parseMsg2.receiveid+parseMsg2.content+"</br>")
                    $('#message').append(
