...
<script>
    let websocket = {
        ...
        stompClient: null,
        init: function() {
            this.id = document.getElementById('stdn_id').textContent;
        ...
        connect: function() {
            var sid = this.id;
            var socket = new SockJS('http://127.0.0.1/onechat');
            ...
            this.stompClient.connect({}, function(frame) {
                ...
                this.subscribe('/send', function(msg) {
                    //Web Socket 접속 후 서버에서 데이터를 받기 위해 준비 하는 함수
                    document.getElementById('all').insertAdjacentHTML(
                        'afterbegin',
                ...
                this.subscribe('/send/' + sid, function(msg) {
                    document.getElementById('me').insertAdjacentHTML(
                        'afterbegin',
                        "<h4>" + JSON.parse(msg.body).sendid + ":" +
                        JSON.parse(msg.body).content + "</h4>"
                    );
                });
                this.subscribe('/send/to/' + sid, function(msg) {
                    document.getElementById('to').insertAdjacentHTML(
                        'afterbegin',
                        "<h4>" + JSON.parse(msg.body).sendid + ":" +
                        JSON.parse(msg.body).content +
                        "</h4>"
        ...
        sendTo: function() {
            var msg = JSON.stringify({
                'sendid': this.id,
                'receiveid': document.getElementById('target').value,
                'content': document.getElementById('totext').value
            });
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 id="click" class="m-0 font-weight-bold text-primary">Websocket</h6>
            </div>
            <div class="card-body">
                <div id="container"></div>
                <div class="col-sm-5">
                    <h1 id="stdn_id">${loginStdn.id}</h1>
...
<%--              <!--begin::Details-->--%>
<%--              <div class="ms-3">--%>
<%--                <a id="click" href="#" class="fs-5 fw-bold text-gray-900 text-hover-primary me-1">Bsfsfsx</a>--%>
